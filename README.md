...
本系统基于支付宝个人商家API账单接口开发，不需要认证，不需要执照，不需要任何费用，不需要后台挂机，有支付宝账号，申请个网页应用即可，一般是30分钟内就能审核通过。
通过支付宝账单收款数据，自动验证订单，并提供两种充值模式：
...
6. **替换收款码**：请手动替换img\zfb.jpg为自己的收款码，否则前端收银台展示的收款码为默认收款码
---
